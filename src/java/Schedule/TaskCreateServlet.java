...
public class TaskCreateServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        Task newTask = new Task(currentUser.getUserID(), plannerId, taskName, taskDate);
        boolean success = plannerDAO.createTask(newTask);

        if (success) {
            response.sendRedirect("TaskListServlet?plannerId=" + plannerId);
        } else {
            request.setAttribute("errorMessage", "Failed to create task");
            // Giữ lại các session và đường dẫn khi chuyển tiếp về createTask.jsp
